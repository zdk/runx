#!/bin/sh
# Token saving benchmark for ls-compact
# Usage: sh bench.sh
...
printf "=== ls-compact — token saving benchmark ===\n\n"
...
for sample in "$SAMPLES_DIR"/*.txt; do
  ...
  # Extract subcommand from filename: ls-la-full → la
  sub=$(echo "$name" | sed 's/^ls-//' | sed 's/-[a-z]*$//')
