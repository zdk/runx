#!/bin/sh
# Token saving benchmark for docker-compact
# Usage: sh bench.sh
...
printf "=== docker-compact — token saving benchmark ===\n\n"
...
for sample in "$SAMPLES_DIR"/*.txt; do
  ...
  # Extract subcommand from filename: docker-ps-full → ps
  sub=$(echo "$name" | sed 's/^docker-//' | sed 's/-[a-z]*$//')
