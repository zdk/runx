#!/bin/sh
# Token saving benchmark for git-compact
# Usage: sh bench.sh
...
printf "=== git-compact — token saving benchmark ===\n\n"
...
for sample in "$SAMPLES_DIR"/*.txt; do
  ...
  # Extract subcommand from filename: git-diff-full → diff
  sub=$(echo "$name" | sed 's/^git-//' | sed 's/-[a-z]*$//')
