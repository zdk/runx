#!/bin/sh
# Token saving benchmark for npm-compact
# Usage: sh bench.sh
...
printf "=== npm-compact — token saving benchmark ===\n\n"
...
for sample in "$SAMPLES_DIR"/*.txt; do
  ...
  # Extract subcommand from filename: npm-install-full → install
  sub=$(echo "$name" | sed 's/^npm-//' | sed 's/-[a-z]*$//')
