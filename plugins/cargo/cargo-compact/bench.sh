#!/bin/sh
# Token saving benchmark for cargo-compact
# Usage: sh bench.sh
...
printf "=== cargo-compact — token saving benchmark ===\n\n"
...
for sample in "$SAMPLES_DIR"/*.txt; do
  ...
  # Extract subcommand from filename: cargo-build-full → build
  sub=$(echo "$name" | sed 's/^cargo-//' | sed 's/-[a-z]*$//')
